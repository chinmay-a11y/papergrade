...
const CFG = {
  ...
  retries:     Number(process.env.SARVAM_RETRIES || 4),
...
async function docAiDigitise(imageBuffer, { language = 'hi-IN', _try = 0 } = {}) {
  try {
    ...
    while (Date.now() < deadline) {
      ...
      last = await httpJson(`/doc-ai/v1/job/${jobId}/status`, { method: 'GET' });
      const st = last?.status || '';
      if (/complet|success|done/i.test(st)) break;
      if (/fail|error/i.test(st)) throw new Error(`doc-ai job ${st}: ${JSON.stringify(last).slice(0, 200)}`);
    }
    if (!/complet|success|done/i.test(last?.status || '')) throw new Error('doc-ai job timed out');
    ...
    if (_try < 1) { await sleep(800); return docAiDigitise(imageBuffer, { language, _try: _try + 1 }); }
    throw err;
  }
}

// Page-level extraction: digitise the WHOLE page once (Doc AI handles full pages
// reliably; narrow per-question band crops do not), then use 105B to split the OCR
// text into per-question answers. Returns { [question_no]: answerText }.
// Two calls of this (page + perturbed page) give the dual-pass confidence signal
// with just two Vision jobs instead of one per crop.
async function extractPageAnswers(pageBuffer, questions, { language = 'hi-IN', pass = 'a', seedKeyBase = '' } = {}) {
  if (MOCK) {
    const map = {};
    for (const q of questions) map[q.question_no] = mockExtract(`Question ${q.question_no}.`, pass, `${seedKeyBase}:${q.question_no}`);
    return map;
  }
  const fullText = await docAiDigitise(pageBuffer, { language });
  return splitAnswers(fullText, questions, { language });
}

async function splitAnswers(fullText, questions, { language = 'hi-IN' } = {}) {
  const schema = { type: 'object', properties: { answers: { type: 'array', items: { type: 'object',
    properties: { question_no: { type: 'integer' }, answer: { type: 'string' } },
    required: ['question_no', 'answer'] } } }, required: ['answers'] };
  const messages = [
    { role: 'system', content:
      "You receive the OCR text of a student's handwritten answer sheet and a list of questions. " +
      "For each question_no, extract ONLY the student's written answer — exclude the printed question " +
      "prompt, the marks like (5), names and headers. Preserve the original language and script. " +
      'If a question is unanswered, use an empty string. Return JSON.' },
    { role: 'user', content: JSON.stringify({
      questions: questions.map(q => ({ question_no: q.question_no, prompt: q.prompt })),
      ocr_text: fullText }) },
  ];
  // Retry the split a few times — a single dropped call otherwise forces the
  // full-page fallback, which makes every answer look identical + low-confidence.
  let parsed = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    try { parsed = await chat(messages, { schema, temperature: 0 }); } catch { parsed = null; }
    if (parsed && Array.isArray(parsed.answers) && parsed.answers.length) break;
    await sleep(500);
  }

  const map = {};
  if (parsed && Array.isArray(parsed.answers)) {
    for (const a of parsed.answers) if (a && a.question_no != null) map[a.question_no] = String(a.answer || '').trim();
  }
  // Fallback: if the split failed entirely, hand each question the full OCR text so
  // grading still runs (keyword/criteria matching per rubric item does the rest).
  for (const q of questions) if (map[q.question_no] == null) map[q.question_no] = parsed ? '' : fullText;
  return map;
}
...
async function gradeAnswer({ rubricItem, extractedText, confidence, seedKey = '' }) {
  ...
  const out = await chat(messages, { schema, temperature: 0 });
  // Forced abstention on any parse/response failure — never crash the pipeline,
  // route to a human instead (on-brand: the model knows when it can't judge).
  if (!out || typeof out.awarded_marks !== 'number') {
    return { awarded_marks: 0, rationale: 'Automatic grading unavailable — routed for human review.', needs_human: true };
  }
  return out;
}

// --- 4b. Booklet compare-grade: answer key vs a whole student booklet -----
// One call: given the teacher's answer key (model answers) and the OCR of the
// student's full booklet, identify each question, compare, and award marks.
// Works for essay/long-answer exams where fixed per-question cropping doesn't apply.
    ...
    properties: {
      answers: {
        ...
        items: {
          ...
          required: ['question_no', 'max_marks', 'awarded_marks', 'rationale', 'student_answer'],
        },
      },
    },
    required: ['answers'],
  };
  if (MOCK) return mockBooklet(questions);
  const messages = [
    { role: 'system', content:
      'You are a fair, strict exam evaluator. You are given the teacher\'s ANSWER KEY (model answers, ' +
      'with the marks for each question) and the OCR of a STUDENT\'s handwritten answer booklet. ' +
      'For EACH question in the key: find the student\'s corresponding answer, compare it to the model ' +
      'answer, and award marks out of that question\'s maximum. Give a one-line rationale and a short ' +
      'lowercase concept tag. If the student did not attempt a question, award 0. If the student\'s text ' +
      'for a question is illegible/garbled, set needs_human=true. Use the marks from the key; never invent ' +
      'a maximum. Set student_answer to the student\'s exact answer text you matched for that ' +
      'question (empty string if unattempted). Return JSON only.' },
    { role: 'user', content: JSON.stringify({
        answer_key: String(answerKeyText || '').slice(0, 12000),
        question_hints: questions.map(q => ({ question_no: q.question_no, max_marks: q.max_marks })),
        student_booklet: String(studentText || '').slice(0, 16000),
      }) },
  ];
  const out = await chat(messages, { schema, temperature: 0 });
  if (!out || !Array.isArray(out.answers)) return { answers: [] };
  return out;
}
...
function mockBooklet(questions) {
  const qs = (questions && questions.length) ? questions
    : require('./seed-rubric').compiled.questions;
  return { answers: qs.map((q, i) => {
    const ease = CONCEPT_EASE[q.concept] ?? 0.65;
    const frac = Math.max(0, Math.min(1, ease + (seeded('bk:' + q.question_no) - 0.5) * 0.4));
    return {
      question_no: q.question_no, max_marks: q.max_marks,
      awarded_marks: Math.round(q.max_marks * frac * 2) / 2,
      student_answer: MOCK_ANSWERS[i % MOCK_ANSWERS.length],
      rationale: frac < 0.5 ? 'Differs from the answer key on the key point.'
        : 'Matches the answer key closely; minor detail missing.',
      concept: q.concept, needs_human: false,
    };
  }) };
}
...
module.exports = {
  isMock, CFG,
  chat, extractHandwriting, extractPageAnswers, docAiDigitise, compileRubric, gradeAnswer, gradeBooklet,
