...
async function processBooklet(scriptId, pageBuffers, { maxPages = Number(process.env.BOOKLET_MAX_PAGES || 20) } = {}) {
  const script = db.getScript(scriptId);
  if (!script) return;
  const rubric = db.getRubric(script.rubric_id) || db.getWorkspaceRubric(script.workspace_id);
  ...
  // Provider select: Claude scans the page images and compares to the key in ONE vision call;
  // Sarvam OCRs each page (Doc AI) then compares via 105B. Set AI_PROVIDER=claude to use Claude.
  const claude = require('./claude');
  const useClaude = process.env.AI_PROVIDER === 'claude' && claude.isReady();

  let graded;
  if (useClaude) {
    db.setScriptStatus(scriptId, 'extracting');
    db.setScriptStatus(scriptId, 'grading');
    try { graded = await claude.gradeBookletVision({ answerKeyText, pageBuffers: pages, questions: compiled.questions, maxPages }); }
    catch (e) { console.error('[booklet claude]', e.message); graded = { answers: [] }; }
  } else {
    // 1. OCR each booklet page (Sarvam Doc AI) and concatenate into the full student text.
    db.setScriptStatus(scriptId, 'extracting');
    const parts = [];
